...
final_revision: '7bb09136372db8602744acf221f8c8d939565b10'
status: 'done'
...
Status: **done** (third review pass — 21 patches, no intent-gap/bad-spec loopback).

**Implemented change:** A second independent follow-up review of story 1.3. Two blind reviewers (adversarial + edge-case) read the full `0665368..HEAD` diff; after verifying each claim against the code, 21 findings were fixed, 1 deferred, 8 rejected (three of them re-raises of findings passes 1 and 2 had already rejected for reasons that still hold). No finding survived verification as high severity. The consequential ones cluster in four places. **Liveness and drain:** every Redis touch in the launch loop was unguarded, so a transient blip escaped `run_backfill` with rows still in flight and `asyncio.run` cancelled them mid-enrichment — the drain is now a `finally`, and the gather tolerates a worker that raised past its own handler (`checkpoint.advance` runs outside it). **Unbounded quota retry:** a quota refusal also sets `budget_exhausted`, which puts the pass out of reach of the stall detector, so a provider refusing everything produced ~96 identical zero-progress `--continuous` passes a day forever; after four consecutive zero-progress refusals the per-minute-throttle reading is now ruled out and the runner waits out the RPD window. **Control-surface truth under handover:** three separate paths let an exiting or displaced runner write over a successor — the stop request was read and cleared *after* `lease.release()`, `--continuous` checked completion before lease loss (so a displaced run reported itself as the one that finished, exit 0), and the budget sleep published state before verifying ownership. **State liveness:** the published state was refreshed once per launch, i.e. once per row, so a single slow row let a live run read back as `idle`; the worker's tick now refreshes the *current* state, which also means it can never stamp `running` over a pause.

**Files changed:**
- `src/core/backfill_runner.py` — launch-loop drain in a `finally` + `return_exceptions` on the gather; the worker tick refreshes the current published state; refresh cadence derived from the control's own TTL (`BackfillControl.refresh_interval_seconds`); `pause_poll_seconds` floored; non-atomic renew honors `EXPIRE`'s 0; advisory meta writes can no longer fail a lease; the non-atomic Redis fallback logs once instead of silently downgrading mutual exclusion and the RPD ceiling.
- `scripts/dev/backfill_gemma.py` — quota-refusal escalation in `--continuous`; lease loss checked before completion; served stop retired before the lease is released; `--reset-quarantine` takes the lease instead of reading the holder; `_sleep_for_reset` renews before publishing and drives its cadences off the wall clock; mutually exclusive command flags; `--reset-margin` validated; whitespace-only key diagnosed as missing; completed run never exits `backing-off`; a drained queue retires the requests aimed at it; `--task-classes` help and module docstring corrected.
- `src/infra/config.py` — cross-field validator: `control_poll_seconds` must stay below `lease_ttl_seconds / 3` (the lease is renewed once per poll).
- `src/tests/unit/test_backfill_control.py`, `test_backfill_gemma_cli.py`, `test_backfill_gemma_completion_cli.py`, `test_config.py` — regressions for every patch; the two CLI fakes' `expire` now returns Redis's 1/0 instead of `None`.
- `src/tests/integration/test_backfill_lua_scripts.py` — four threads racing `try_consume` against the real test-stack Redis, locking AC-1's never-exceed guarantee under actual contention (the unit tests only ever reserved single-threaded).
- `docs/features/v0.13-s1.3-backfill-runner-control-core.md` — second follow-up-review section.

**Review findings breakdown:** 21 patches applied (medium 6, low 15). 0 intent-gap, 0 bad-spec. 1 deferred (a provider throttle arriving as connection resets/timeouts is charged to the row and can permanently quarantine good properties — pre-existing ledger behaviour whose fix needs a deliberate heuristic). 8 rejected, listed with reasons in the triage log.

**Verification:** `bash scripts/agent/validate.sh fast` → green (1620 unit passed, 1 skipped; pre-commit clean). `bash scripts/agent/validate.sh backend` → green (1620 unit, 90 integration incl. the new concurrent-reservation test against the test-stack Redis, 32 contract; alembic's PostGIS-system-table diff is the known informational warning). `validate-ai.sh` was **not** re-run: this pass changed no AI prompt or client code (`src/adapters/ai/client.py` is untouched — the gate ran green in pass 2 when it last changed). `bash scripts/agent/validate.sh all` runs at merge via `finish-feature.sh`.

**Residual risks:** The new `control_poll_seconds < lease_ttl_seconds / 3` validator will reject a previously-accepted `app_config.yaml` at startup — intentional, since the rejected range lets the lease lapse under a paused runner, but it is a hard failure rather than a warning. The quota escalation trades responsiveness for restraint: a genuine per-minute throttle that somehow persisted past four back-off cycles now costs a full RPD-window wait instead of a 15-minute one. The launch loop's own lease renewal and control reads remain deliberately unguarded against transient Redis errors (only the drain is protected now), so a blip still ends the pass — resumable via the checkpoint, and loud rather than silent. And the deferred single-row-outliving-the-lease item is unchanged: the state-key twin of it is narrowed by the worker tick but not closed, since both want the same background renewer.

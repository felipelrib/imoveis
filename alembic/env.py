"""Alembic environment — runs migrations online or offline.

Updated to:
- Add src/ to sys.path so src.* imports resolve
- Import ALL model modules so every table is registered in Base.metadata
- Use DATABASE_URL env var override (consistent with infra.config)
"""

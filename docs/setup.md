...
`requirements.txt` is a **fully pinned lockfile** generated by [`pip-compile`](https://pip-tools.readthedocs.io/) (from [`pip-tools`](https://pypi.org/project/pip-tools/)) — every direct *and* transitive dependency is pinned to an exact version, so `pip install -r requirements.txt` resolves identically in local dev, CI, and the Docker images (`Dockerfile.api`, `Dockerfile.worker`) regardless of when it runs.

The hand-edited source of truth is `requirements.in` (loose/top-level dependencies, same file CI/Docker used before this lockfile existed). Never hand-edit `requirements.txt` — regenerate it from `requirements.in` instead. Use the same Python version as CI/Docker (**3.11** — see `.github/workflows/ci.yml` and `Dockerfile.api`/`Dockerfile.worker`) so resolved wheels match what actually ships:

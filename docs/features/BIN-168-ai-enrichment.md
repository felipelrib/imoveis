...
> Feature branch: `feat/ai-enrichment` · Linear: `BIN-168` · Status: implemented
...
- **Deal verdict synthesis**: Combines statistical, visual, and sentiment analyses into a single locale-aware "punchline" sentence (English default; `pt-BR` when UI locale is Portuguese — see [BIN-101-localize-ai-tags-verdicts.md](BIN-101-localize-ai-tags-verdicts.md) / BIN-101). Uses LLM when available, falls back to a deterministic template (`template_deal_verdict`). Details: [BIN-12-deal-summary-enrichment.md](BIN-12-deal-summary-enrichment.md).

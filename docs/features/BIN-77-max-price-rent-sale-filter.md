...
```
 ...
 docs/features/BIN-77-max-price-rent-sale-filter.md             | NEW — this note
```
...
- Saved-search camelCase vs snake_case: fixed in [BIN-100](https://linear.app/felipelrib/issue/BIN-100) / `docs/features/BIN-100-locale-aware-filters.md` (API aliases + `toSavedSearchWire`). [BIN-109](https://linear.app/felipelrib/issue/BIN-109) adds create-path + Playwright regressions for `price_type` / `max_price` round-trip.

...
```
 ...
 docs/features/BIN-152-add-missing-db-indexes.md                            | NEW — this doc
```
...
- Re-check `EXPLAIN (ANALYZE, BUFFERS)` on the populated primary DB once volume is materially higher than the ~26k rows measured here, to confirm the planner is actually choosing the new indexes for the hot-path queries (an empty/small worktree DB will always prefer Seq Scan, same caveat as `docs/features/BIN-121-gist-index-properties-location.md`).

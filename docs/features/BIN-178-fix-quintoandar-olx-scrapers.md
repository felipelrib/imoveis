...
> Feature branch: `fix/quintoandar-olx-scrapers` · Linear: `BIN-178` · Status: implemented
...
```
 ...
 docs/features/BIN-178-fix-quintoandar-olx-scrapers.md | NEW — this note
```
...
- Live dry-run (`scripts/dev/test_scraper_dryrun.py`) only covers QuintoAndar **normalize**, not `scrape_listings` / `cfg.dedup`. Regression gates for config+task wiring and OLX Flight HTML live in unit CI — see `docs/features/BIN-179-scraper-regression-gates.md`.

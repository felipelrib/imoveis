...
- **BIN-38 (2026-07-23):** Status/docs reconcile closed — FR-15 complete; no remaining scope split into a v0.5 story. See `BIN-38-semantic-search-reconcile.md`.
- **Model (current):** Ollama `bge-m3` at **1024-d** (BIN-73); the nomic / 768-d notes above are historical.
- **Locale (BIN-102 / `BIN-102-semantic-search-locale.md`):** Prefer querying in listing language (PT). English `q=` is expanded with a small PT↔EN housing lexicon (`penthouse`↔`cobertura`, `backyard`↔`quintal`, …) before embed. Index text remains scraped title+description only — no re-embed for language. Description scrape/persist restored in BIN-105 (`BIN-105-listing-description-scrape.md`).

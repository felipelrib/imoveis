...
> Feature branch: `feat/watchlist` · Linear: `BIN-10` · Status: implemented
...
- Owner scoping ([BIN-45](https://linear.app/felipelrib/issue/BIN-45) / `docs/features/BIN-45-owner-scoped-favourites-searches-watchlist.md`): `/watchlist` requires `X-API-Key` and scopes by `owner` (JWT `user_id` removed). Alert evaluation remains system-wide by `property_id`.

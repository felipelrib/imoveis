...
```
 ...
 docs/features/BIN-74-scraper-coverage-location-source-filter.md | NEW — this note
```
...
- ~~Atomic QuintoAndar windows that remain ≥12 after house-type fan-out still truncate at SSR; no deeper pagination without a non-SSR client.~~ Addressed in **BIN-111** / `docs/features/BIN-111-quintoandar-ssr-ceiling-metrics.md` (hard ceiling documented + truncation metrics; no non-SSR client).

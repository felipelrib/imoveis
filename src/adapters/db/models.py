...
class MetricsScoring(Base):
    ...
    # BIN-83: listing-type scores (nullable when that type is absent)
    stat_score_rent = Column(Float)
    stat_score_sale = Column(Float)
    z_score_rent = Column(Float)
    z_score_sale = Column(Float)
    percentile_rank_rent = Column(Float)
